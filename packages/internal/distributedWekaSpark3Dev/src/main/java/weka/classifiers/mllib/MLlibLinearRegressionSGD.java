...
public class MLlibLinearRegressionSGD extends MLlibClassifier {
  ...
  @OptionMetadata(displayName = "Regularization",
    description = "Regularization to use: None (OLS), L1 (Lasso), L2 (ridge). "
      + "Default = None.",
    commandLineParamName = "regularization",
   ...
   */
  @OptionMetadata(displayName = "Mini batch fraction", description = "Fraction "
  ...
  public void learnModel(JavaRDD<Instance> wekaData,
    ...
    if (m_regularization == Regularization.None) {
      LinearRegressionWithSGD linearRegressionWithSGD =
        new LinearRegressionWithSGD(m_stepSize, m_numIterations,
          m_regularizationParam, m_miniBatchFraction);
      m_model = linearRegressionWithSGD.run(mlLibData.rdd());
    } else if (m_regularization == Regularization.L1) {
      LassoWithSGD lassoWithSGD = new LassoWithSGD(m_stepSize, m_numIterations,
        m_regularizationParam, m_miniBatchFraction);
      m_model = lassoWithSGD.run(mlLibData.rdd());
    } else {
      RidgeRegressionWithSGD ridgeRegressionWithSGD =
        new RidgeRegressionWithSGD(m_stepSize, m_numIterations,
          m_regularizationParam, m_miniBatchFraction);
      m_model = ridgeRegressionWithSGD.run(mlLibData.rdd());
    }
  }
